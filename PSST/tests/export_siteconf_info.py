...
def add_condor_config_var(glidein_config, name=None, kind="C",
                          value=None, publish=True, condor_name=None):
    if (name is None) or (value is None):
        print ("Invalid condor configuration specified (name=%s; value=%s)"
               % (str(name), str(value)))
        return

    if 'CONDOR_VARS_FILE' not in glidein_config:
        print ("Warning: Missing condor vars file from configuration; ignoring\
              (%s=%s)" % (name, value))
        return

    has_whitespace = re.compile("\s")
    if has_whitespace.search(name):
        print("Ignoring specified name as it contains whitespace (name=%s)."
              % name)
        return
    if has_whitespace.search(value):
        print("Ignoring specified value as it contains whitespace (value=%s)."
              % name)
        return
    if condor_name and has_whitespace.search(condor_name):
        print("Ignoring specified HTCondor variable name as it contains \
               whitespace (condor_name=%s)." % condor_name)
        return
    ...
    tempfd = tempfile.NamedTemporaryFile(dir=vars_dir, prefix=vars_file,
                                         delete=False)
    try:
        fd = open(fname, "r")
